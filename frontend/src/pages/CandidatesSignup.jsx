...
const CandidatesSignup = () => {
  ...
  const isWorkerMode = searchParams.get("worker") === "true";
  ...
  const userSignupHanlder = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // If worker mode, redirect to service profile setup
        if (isWorkerMode) {
          navigate("/service-profile?setup=true");
        } else {
          navigate("/");
        }
      } else {
          ...
          <div className="w-full max-w-md border border-gray-200 rounded-lg p-6">
            <div className="text-center mb-6">
              <h1 className="text-2xl font-semibold text-gray-700 mb-1">
                {isWorkerMode ? "Join as Service Worker" : "Candidate Signup"}
              </h1>
              <p className="text-sm text-gray-600">
                {isWorkerMode
                  ? "Create your account to apply for service jobs"
