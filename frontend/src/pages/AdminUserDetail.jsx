...
const AdminUserDetail = () => {
    ...
    const fetchUserDetail = async () => {
        try {
            const { data } = await axios.get(
                `${backendUrl}/admin/user/${id}/resume`,
    ...
    const handleReparseResume = async () => {
        if (!user?.resume) {
            toast.error("User has no resume uploaded");
            return;
        }
        ...
        try {
            const { data } = await axios.post(
                `${backendUrl}/admin/user/${id}/reparse`,
                {},
                { headers: { token: adminToken } }
            );
            if (data.success) {
            ...
            toast.error(error.response?.data?.message || "Failed to re-parse resume");
        } finally {
            setReparsing(false);
        }
    };
    ...
    }, [adminToken, id, backendUrl]);

    // Check if resume data is empty (needs re-parsing)
    const hasExtractedData = user && (
                        ...
                        {user.resume && (
                            <div className="flex flex-wrap items-center gap-3 mt-4">
                                ...
                                <button
                                    ...
                                    disabled={reparsing}
                                    className="inline-flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700 transition disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    <RefreshCw size={18} className={reparsing ? "animate-spin" : ""} />
