...
const ApplyJob = () => {
  ...
  // Service categories that require service profile instead of IT resume
  ...
  const applyJob = async (jobId) => {
    try {
      ...
      // Check if this is a service job or IT job
      if (isServiceJob(jobCategory)) {
        // Service job - requires service profile
        if (!userData?.serviceProfile?.isActive) {
          navigate("/service-profile");
          return toast.error("Please create your Service Profile to apply for this job");
        }
      } else {
        // IT job - requires IT resume
        if (!userData?.resume) {
